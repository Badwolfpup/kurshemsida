...
import { addDays, addWeeks, format, getDay, isBefore, startOfDay, startOfWeek, subWeeks } from 'date-fns';
...
class FourDayView extends React.Component<any> {
  render() {
    ...
    return React.createElement(TimeGrid, {
      ...props, range, eventOffset: 15, localizer: loc,
...
const isWithinWorkHours = (start: Date, end: Date) => {
  ...
  return s >= WORKDAY_START_HOUR * 60 && e <= WORKDAY_END_HOUR * 60 && start < end;
};
...
function generate30MinOptions(fromH: number, fromM: number, toH: number, toM: number) {
...
function CoachBookingView() {
  ...
  const [bookingDetailsMode, setBookingDetailsMode] = useState<'view' | 'reschedule'>('view');
  ...
  const adminColorMap = useMemo(() => {
    ...
    admins.forEach((admin, i) => map.set(admin.id, ADMIN_COLORS[i % ADMIN_COLORS.length]));
    return map;
  }, [admins]);

  const adminNameMap = useMemo(() => {
    const map = new Map<number, string>();
    admins.forEach((admin) => map.set(admin.id, `${admin.firstName} ${admin.lastName}`));
    return map;
  }, [admins]);
  ...
  const studentNameMap = useMemo(() => {
    const map = new Map<number, string>();
    allUsers.filter((u) => u.authLevel === 4).forEach((s) => map.set(s.id, `${s.firstName} ${s.lastName}`));
    return map;
  }, [allUsers]);
  ...
  const loadData = async () => {
    try {
      ...
      setAvailabilities(availData.filter((a: Availability) => isWithinWorkHours(new Date(a.startTime), new Date(a.endTime))));
  ...
  const events = useMemo((): CalendarEvent[] => {
    ...
    for (const avail of filteredAvailabilities) {
      ...
      for (let i = 0; i < freeSegs.length; i++) {
        ...
        result.push({
          ...
          allDay: false,
          resource: { type: 'free', availability: avail, adminId: avail.adminId, adminName, color, freeStart: seg.start, freeEnd: seg.end },
        });
      }
    }
    ...
    for (const b of bookings) {
      ...
      result.push({
        ...
        allDay: false,
        resource: { type: isOwn ? 'my-booking' : 'other-booking', booking: b, adminId: b.adminId, adminName, color },
      });
    }
  ...
  const FreeEventComponent = useMemo(() => {
    ...
    return ({ event }: { event: any }) => {
      ...
      const isPast = isBefore(startOfDay(calEvent.start), startOfDay(new Date()));
      ...
      const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        const clickedTime = new Date(freeStart.getTime() + fraction * durationMinutes * 60000);
        openBookingDialog(calEvent.resource.availability!, freeStart, freeEnd, clickedTime);
      };

      return (
        <div onClick={handleClick} style={{ height: '100%', width: '100%', cursor: isPast ? 'default' : 'pointer' }}>
          {calEvent.title}
        </div>
      );
    };
  ...
  const eventStyleGetter = (event: CalendarEvent) => {
    ...
    if (event.resource.type === 'other-booking') {
      return { style: { ...base, backgroundColor: '#ef4444', color: '#fff', opacity: 0.6, cursor: 'default' } };
    }
    return {};
  };
    ...
    <div className="p-6 lg:p-10 max-w-6xl mx-auto">
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription asChild>
              <div>
                {selectedFreeSlot && (
                  <>
                    <p className="mt-2"><strong>Admin:</strong> {adminNameMap.get(selectedFreeSlot.availability.adminId) || 'Okänd'}</p>
                    <p><strong>Datum:</strong> {format(selectedFreeSlot.start, 'yyyy-MM-dd')}</p>
                    <p><strong>Tillgänglig:</strong> {format(selectedFreeSlot.start, 'HH:mm')} – {format(selectedFreeSlot.end, 'HH:mm')}</p>
                  </>
                )}
              </div>
            </DialogDescription>
          ...
          {!meetingType ? (
            ...
            <>
                  ...
                  <Select
                    ...
                    onValueChange={(val) => {
                      ...
                      if (endHour * 60 + endMinute <= newStartTotal) {
                        const next = timeOptions.find((o) => o.hour * 60 + o.minute > newStartTotal);
              ...
              <DialogFooter>
                <Button variant="outline" onClick={() => setMeetingType(null)}>Tillbaka</Button>
                <Button onClick={handleBook} disabled={meetingType === 'followup' && (!studentId || students.length === 0)}>
                  Bekräfta bokning
                </Button>
              </DialogFooter>
            </>
      ...
      <Dialog open={showBookingDetails} onOpenChange={(open) => { setShowBookingDetails(open); if (!open) { setBookingDetailsMode('view'); setFetchedParentAvail(null); } }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {bookingDetailsMode === 'reschedule' ? 'Ändra tid' :
            ...
            <DialogDescription asChild>
              <div className="space-y-2 mt-2">
                {selectedBooking && bookingDetailsMode === 'view' && (
                  <>
                    <p><strong>Admin:</strong> {adminNameMap.get(selectedBooking.adminId) || 'Okänd'}</p>
                    <p>
                      <strong>Tid:</strong>{' '}
                    ...
                    {selectedBooking.studentId && (
                      <p><strong>Student:</strong> {studentNameMap.get(selectedBooking.studentId) || `ID ${selectedBooking.studentId}`}</p>
                    )}
          ...
          {bookingDetailsMode === 'view' && selectedBooking?.status === 'accepted' && !isBookingInPast(selectedBooking) && (
          ...
          <DialogFooter>
            {bookingDetailsMode === 'reschedule' ? (
              ...
              <Button variant="outline" onClick={() => setShowBookingDetails(false)}>Stäng</Button>
            )}
          </DialogFooter>
        </DialogContent>
      </Dialog>
