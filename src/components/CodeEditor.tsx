...
function getTabsForCategory(category: string): TabConfig[] {
  switch (category) {
    ...
    case 'react':
      return [{ id: 'js', label: 'JSX' }];
    case 'python':
      return [{ id: 'js', label: 'Python' }];
    case 'csharp':
      return [{ id: 'js', label: 'C#' }];
    default:
      return [{ id: 'html', label: 'HTML' }];
  }
}
...
function splitStarterCode(
  ...
  if (category === 'html' || category === 'react') {
    result.html = code;
    return result;
  }
  if (category === 'python' || category === 'csharp') {
    result.js = code;
    return result;
  }
...
export default function CodeEditor({
  ...
  const isWebLanguage = ['html', 'html_css', 'html_css_js', 'react'].includes(
    language
  );

  const updateTabCode = (tab: TabId, code: string) =>
    setTabCode((prev) => ({ ...prev, [tab]: code }));
  const getCombinedCode = useCallback(
    () => combineCode(tabCode, language),
    [tabCode, language]
  );
  ...
  const runCode = useCallback(() => {
    ...
    if (isWebLanguage) {
      ...
      if (doc.includes('<head>'))
        ...
        doc = doc.replace('<html>', `<html><head>${consoleCapture}</head>`);
      ...
      if (expectedOutput) {
        ...
        setTimeout(() => {
          if (iframeRef.current?.contentDocument) {
            ...
            const match =
              norm(body).includes(norm(expectedOutput)) ||
              norm(combined).includes(norm(expectedOutput));
            setIsCorrect(match);
      ...
      setConsoleOutput([
        'Körning av ' +
          language +
          ' stöds inte i webbläsaren. Kopiera koden och kör den lokalt.',
      ]);
      setShowPreview(true);
      setShowConsole(true);
    }
  ...
  const resetCode = () => {
    setTabCode(splitStarterCode(starterCode || '', language));
  ...
  const revealNextHint = () => {
    if (revealedHints < hints.length) setRevealedHints((p) => p + 1);
  };
  const copyCode = async () => {
    await navigator.clipboard.writeText(getCombinedCode());
    setCopied(true);
    ...
    >
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          >
            <Play className="h-3.5 w-3.5" /> Kör
          </Button>
          <Button
            size="sm"
            variant="outline"
        ...
        <div className="flex items-center gap-2">
          {hints.length > 0 && (
            <Button
              ...
              className="gap-1.5"
            >
              <Lightbulb className="h-3.5 w-3.5" /> Ledtråd ({revealedHints}/
              {hints.length})
            </Button>
          )}
          {solutionCode && (
          ...
          {explanation && (
            ...
            >
              {showExplanation ? 'Dölj förklaring' : 'Förklaring'}
            </Button>
          )}
          {isCorrect !== null && (
            <Badge
            ...
            >
              {isCorrect ? (
                <>
                  <CheckCircle className="h-3.5 w-3.5 mr-1" /> Rätt!
                </>
              ) : (
                'Försök igen'
              )}
      ...
      <div
        className={`grid gap-3 ${isFullscreen ? 'grid-cols-2 h-[calc(100vh-200px)]' : 'grid-cols-1 lg:grid-cols-2'}`}
      >
            ...
            <div className="flex border-b border-border bg-[#252526] rounded-t-lg">
              {editorTabs.map((tab) => (
                <button
                  ...
                  className={`px-4 py-2 text-xs font-medium transition-colors border-b-2 ${activeTab === tab.id ? 'border-primary text-primary bg-[#1e1e1e]' : 'border-transparent text-[#858585] hover:text-[#d4d4d4] hover:bg-[#2d2d30]'}`}
                >
                  {tab.label}
                </button>
              ))}
          ...
          <textarea
            ...
            onKeyDown={(e) => {
              if (e.key === 'Tab') {
                ...
                updateTabCode(
                  activeTab,
                  tabCode[activeTab].substring(0, s) +
                    '  ' +
                    tabCode[activeTab].substring(end)
                );
                setTimeout(() => {
        ...
        <div className="flex flex-col">
          <div className="flex border-b border-border bg-muted/50 rounded-t-lg">
            <button
              className={`px-4 py-2 text-xs font-medium transition-colors border-b-2 ${!showConsole ? 'border-primary text-primary' : 'border-transparent text-muted-foreground'}`}
              onClick={() => setShowConsole(false)}
            >
              {isWebLanguage ? 'Förhandsgranskning' : 'Utskrift'}
            </button>
            <button
              className={`px-4 py-2 text-xs font-medium transition-colors border-b-2 flex items-center gap-1.5 ${showConsole ? 'border-primary text-primary' : 'border-transparent text-muted-foreground'}`}
          ...
          <div
            className={`border border-border border-t-0 rounded-b-lg overflow-hidden flex-1`}
          >
            {!showConsole ? (
              isWebLanguage ? (
                <div
                  className={`bg-white ${isFullscreen ? 'h-full' : 'min-h-[300px]'}`}
                >
                  {showPreview ? (
                    <iframe
                      ref={iframeRef}
                      title="preview"
                      className={`w-full border-0 ${isFullscreen ? 'h-full' : 'min-h-[300px]'}`}
                ...
                <pre
                  className={`bg-[#1e1e1e] text-[#d4d4d4] font-mono text-sm p-4 overflow-auto whitespace-pre-wrap ${isFullscreen ? 'h-full' : 'min-h-[300px]'}`}
                >
                  {consoleOutput.length > 0
                    ? consoleOutput.join('\n')
              ...
              <div
                className={`bg-[#1e1e1e] p-3 overflow-auto ${isFullscreen ? 'h-full' : 'min-h-[300px] max-h-[300px]'}`}
              >
                {consoleOutput.length === 0 ? (
                  <p className="text-[#858585] text-xs font-mono">
                    Konsolen är tom. Kör koden för att se utskrifter.
                  </p>
                ) : (
                  <div className="space-y-1">
                    {consoleOutput.map((line, i) => (
                      <div
                        key={i}
                        className={`text-xs font-mono ${line.startsWith('Error:') ? 'text-red-400' : line.startsWith('Warning:') ? 'text-yellow-400' : 'text-[#d4d4d4]'}`}
                      >
      ...
      {showExplanation && explanation && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 text-sm">
          ...
          <p className="text-blue-900 dark:text-blue-200 mt-1 whitespace-pre-wrap">
            {explanation}
          </p>
        </div>
      )}
      ...
      {showSolution && solutionCode && (
        <div className="space-y-1">
          ...
          <pre className="bg-[#1e1e1e] text-[#d4d4d4] font-mono text-sm p-4 rounded-lg border border-border overflow-x-auto whitespace-pre-wrap">
            {solutionCode}
          </pre>
        </div>
      )}
